...
var insertUser = function (data) {
    ...
    if('api_key' in data){
        if(data.api_key !="" && data.api_key !=null && data.api_key !="undefined"){
            db.collection('users_task_1').insertOne({name:data.name, password:passwordHash, email:data.add_email,status:1,created_at:localIST()});
            defer.resolve(1);  
...
var deleteUser = function (id) {
    ...
    db.collection('users_task_1').updateOne({_id:user_id}, {$set: {status: 0, deleted_at: localIST()}},function (err, result){
        if(err){ 
            dfd.reject(err);
...
var checkLogin = function (username,password){
    var dfd = Q.defer();
    var db = mongoUtil.getDb().db(config.db_name);
    db.collection('users_task_1').find({email: username, status: 1}).toArray(function (err, result) {
        if (err) {
            dfd.reject(err);
            return console.error(err); 
...
var viewUser = function (id){
    ...
    var db = mongoUtil.getDb().db(config.db_name);
    var user_id=mongoUtil.ObjectID(id);
    var collection = db.collection('users_task_1').find({_id:user_id,status:1});
...
var checkNewUser = function (name){
    var dfd = Q.defer();
    var db = mongoUtil.getDb().db(config.db_name);
    var collection=db.collection('users_task_1').find({email:name,status:1});
    collection.toArray(function (err, result) {
        if(err){ 
            dfd.reject(err);
            return console.error(err); 
